<?php
        if (isset($_POST['submit']))
        {
            ...
            $GloveColour         = $_POST['GloveColourName'];
...
$connect->beginTransaction();
{
  ...
  // echo "<br>";
  //echo $Customer;
  $query = "{CALL SP_AddNewSFGRecord(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)}";
  $stmt = $connect->prepare($query);
  $stmt->bindParam(1, $Plant);
  $stmt->bindParam(2, $BatchNumber, PDO::PARAM_INT);
  $stmt->bindParam(3, $Customer);
  $stmt->bindParam(4, $Brand);
  $stmt->bindParam(5, $LotNumber, PDO::PARAM_INT);
  $stmt->bindParam(6, $LotCount, PDO::PARAM_INT);
  $stmt->bindParam(7, $InspectionPlan);
  $stmt->bindParam(8, $CartonQuantity, PDO::PARAM_INT);
  $stmt->bindParam(9, $ReinspectionFlag, PDO::PARAM_INT);
  $stmt->bindParam(10, $ConvertFlag, PDO::PARAM_INT);
  $stmt->bindParam(11, $ProductionDateWLine_JSON);
  $stmt->bindParam(12, $PackingDate);
  $stmt->bindParam(13, $PackingShift, PDO::PARAM_INT);
  $stmt->bindParam(14, $GloveCode);
  $stmt->bindParam(15, $GloveColour);
  $stmt->bindParam(16, $GloveSurface);
  $stmt->bindParam(17, $GloveProductType);
  $stmt->bindParam(18, $GloveSize);
  $stmt->bindParam(19, $CartonNum_JSON );
  $stmt->bindParam(20, $InspectionCount, PDO::PARAM_INT);
  $stmt->bindParam(21, $RecordCreatedDateTime);
  $stmt->bindParam(22, $InspectionUserID, PDO::PARAM_INT);
  $stmt->bindParam(23, $RecordStatusFlag, PDO::PARAM_INT);
  $stmt->bindParam(24, $VerifierID);
  $stmt->bindParam(25, $VerifiedDate);
  $stmt->bindParam(26, $RecordIDPreset);
  $stmt->bindParam(27, $InstrumentResult_JSON);
  $stmt->bindParam(28, $TestResult_JSON );
  $stmt->bindParam(29, $DefectResult_JSON );
  $stmt->bindParam(30, $AQLResult_JSON );
  $stmt->bindParam(31, $SampleSizeResult_JSON);
  $stmt->bindParam(32, $UDResultKey);
